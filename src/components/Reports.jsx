...
export default function Reports() {
  ...
  const reportedPosts = posts.filter(p => reportedPostIds.includes(p.id))

  // Get reporters for each post
  const getReporters = (postId) => reports.filter(r => r.postId === postId).map(r => r.reporter)
          ...
          <article key={p.id} className="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow overflow-hidden">
            <div className="flex">
              ...
              <div className="flex-1 p-4">
                ...
                <p className="text-sm text-red-600 mb-2">Reported by: {getReporters(p.id).join(', ')}</p>
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    onClick={() => {
                      ...
                      reports.filter(r => r.postId === p.id).forEach(r => deleteReport(r.id))
                    }}
